...
export const SERVICES: Record<string, ServiceConfig> = {
  ...
  CUSTOM_AUTHORITIES: {
    ...
    description: 'ERCA - Customs clearance, export duties, and revenue collection',
    type: 'consortium',
    order: 6,
    hasAuth: false, // ERCA users authenticate via ESW API, not this service
  },
  SHIPPING_LINE: {
    id: 'shipping-line',
    name: 'Shipping Line',
...
export const ESW_ENDPOINTS = {
  // Submissions
  SUBMISSIONS: '/api/esw/submissions',
  SUBMISSION_DETAILS: (id: string) => `/api/esw/submissions/${id}`,
  SUBMISSION_AGENCIES: (id: string) => `/api/esw/submissions/${id}/agencies`,
  SUBMISSION_CERTIFICATES: (id: string) => `/api/esw/submissions/${id}/certificates`,
  
  // Agency approvals
  AGENCY_APPROVE: (submissionId: string, agencyCode: string) => 
    `/api/esw/submissions/${submissionId}/agencies/${agencyCode}/approve`,
  AGENCY_PENDING: (agencyCode: string) => `/api/esw/agencies/${agencyCode}/pending`,
  AGENCY_STATS: (agencyCode: string) => `/api/esw/agencies/${agencyCode}/stats`,
  
  // Agencies
  AGENCIES: '/api/esw/agencies',
  
  // Certificates
  CERTIFICATE_DETAILS: (id: string) => `/api/esw/certificates/${id}`,
  CERTIFICATE_DOWNLOAD: (id: string) => `/api/esw/certificates/${id}/download`,
  CERTIFICATE_VERIFY: (certificateNumber: string) => `/api/esw/certificates/verify/${certificateNumber}`,
  
  // Statistics
  STATISTICS: '/api/esw/statistics',
  STATS: '/api/esw/stats',
  
  // Export-specific
  EXPORT_SUBMISSION: (exportId: string) => `/api/esw/exports/${exportId}/submission`,
} as const;
