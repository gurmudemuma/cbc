...
const ectaPreRegistrationService = {
   ...
   */
  getQualificationStatusByUsername: async (username) => {
    // Gateway handles auth routes at /api/auth
    const response = await apiClient.get(`/api/auth/qualification-status/${username}`);
    return response.data;
  },

  /**
   * Register new exporter user account (public endpoint for login page)
   */
  registerUserAccount: async (userData) => {
    // Gateway handles auth routes at /api/auth
  ...
  getExporterDashboard: async (exporterId) => {
    const response = await apiClient.get(`/api/ecta/preregistration/dashboard/exporter/${exporterId}`);
    return response.data;
  },
  ...
  getExporterDashboardByTin: async (tin) => {
    const response = await apiClient.get(`/api/ecta/preregistration/dashboard/exporter/tin/${tin}`);
    return response.data;
  },
