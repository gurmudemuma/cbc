...
const ApplicationTracking = ({ user, org }: ApplicationTrackingProps): JSX.Element => {
  ...
  const handleDownloadCertificate = async (application: any) => {
    if (!application.certificateId || !application.filePath) {
      setSnackbar({ open: true, message: 'Certificate PDF not available', severity: 'error' });
      return;
    }

    setDownloading(application.id);
    try {
      if (application.certificateType === 'competence') {
        await ectaService.downloadCompetenceCertificate(application.certificateId);
        setSnackbar({ open: true, message: 'Competence certificate downloaded successfully', severity: 'success' });
      } else if (application.certificateType === 'license') {
        await ectaService.downloadExportLicense(application.certificateId);
        setSnackbar({ open: true, message: 'Export license downloaded successfully', severity: 'success' });
      }
    } catch (error: any) {
  ...
  const canDownloadCertificate = (application: any) => {
    return (
      application.status === 'APPROVED' &&
      application.certificateId &&
      application.filePath &&
      (application.certificateType === 'competence' || application.certificateType === 'license')
    );
  };
                  ...
                  {applications.map((application) => (
                    <TableRow key={application.id} hover>
                      ...
                      <TableCell>
                        ...
                        {canDownloadCertificate(application) ? (
                        ...
                        ) : application.status === 'APPROVED' && application.certificateType ? (
                          <Tooltip title="PDF Pending">
