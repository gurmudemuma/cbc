...
const ExporterPreRegistration = ({ user, org }: ExporterPreRegistrationProps): JSX.Element => {
  ...
  const handleDownloadCompetenceCertificate = async () => {
    try {
      if (!qualificationStatus?.competenceCertificate?.certificate_id) return;
      setDownloading('competence');
      // Import this dynamically or at the top if adding import
      const { downloadCompetenceCertificate } = await import('../services/ecta.service');
      await downloadCompetenceCertificate(qualificationStatus.competenceCertificate.certificate_id);
  ...
  const handleDownloadExportLicense = async () => {
    try {
      if (!qualificationStatus?.exportLicense?.license_id) return;
      setDownloading('license');
      const { downloadExportLicense } = await import('../services/ecta.service');
      await downloadExportLicense(qualificationStatus.exportLicense.license_id);
      setSuccess('License downloaded successfully');
    } catch (err) {
      console.error('Download error:', err);
      setError('Failed to download license');
    } finally {
      setDownloading(null);
    }
  };
              ...
              <Card sx={{ mb: 3 }}>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    {(qualificationStatus.competenceCertificate?.valid || qualificationStatus.competenceCertificate?.status === 'ACTIVE') && (
                      <Button
                        variant="contained"
                        color="primary"
                        startIcon={downloading === 'competence' ? <CircularProgress size={20} color="inherit" /> : <Description />}
              ...
              <Card sx={{ mb: 3 }}>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    {(qualificationStatus.exportLicense?.valid || qualificationStatus.exportLicense?.status === 'ACTIVE') && (
                      <Box sx={{ ml: 2 }}>
                        <Button
                          variant="contained"
                          color="primary"
                          startIcon={downloading === 'license' ? <CircularProgress size={20} color="inherit" /> : <Description />}
                  ...
                  {qualificationStatus.isQualified && (
                    <Alert severity="success" sx={{ mt: 2 }}>
                      ✅ You are qualified to apply for an export license!
                    </Alert>
                  )}
                </CardContent>
              </Card>
