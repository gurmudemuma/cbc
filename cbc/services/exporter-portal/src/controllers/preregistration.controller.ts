...
export class ExporterPreRegistrationController {
  ...
  public applyForCompetenceCertificate = async (
    ...
    try {
      ...
      // Insert directly into database (same pattern as registerLaboratory and registerTaster)
      const query = `
        INSERT INTO competence_certificates (
          exporter_id, application_reason, additional_documents, facility_description,
          status, application_date, applicant_user_id
      ...
      logger.info('Competence certificate application submitted', { exporterId, applicationId: application.certificate_id });

      res.status(201).json({
        success: true,
        message: 'Competence certificate application submitted. ECTA will schedule facility inspection.',
        data: {
          applicationId: application.certificate_id,
          exporterId,
          status: 'PENDING',
  ...
  public getMyDashboard = async (
    ...
    try {
      ...
      // Note: Raw database rows use snake_case, so we need to access them correctly
      const dashboard = {
        identity: {
        ...
        documents: {
          registrationNumber: profile.registrationNumber,
          laboratoryCertificationNumber: laboratory?.certification_number || null,
          tasterCertificateNumber: taster?.proficiency_certificate_number || null,
          competenceCertificateNumber: competenceCert?.certificate_number || null,
          competenceCertificateId: competenceCert?.certificate_id || null,
          exportLicenseNumber: license?.license_number || null,
          exportLicenseId: license?.license_id || null,
          eicRegistrationNumber: license?.eic_registration_number || null,
        },
        validation: {
    ...
    try {
      ...
      competenceCerts.rows.forEach(cert => {
        applications.push({
          ...
          certificateType: 'competence',
          filePath: cert.file_path,
          fileSize: cert.file_size_bytes,
        });
      });
      ...
      licenses.rows.forEach(license => {
        applications.push({
          ...
          // License download info
          certificateId: license.license_id,
          certificateType: 'license',
          filePath: license.file_path,
          fileSize: license.file_size_bytes,
        });
      });
  ...
  public getESWPrefillData = async (
    ...
    try {
      ...
      const [laboratories, tasters, competenceCerts, licenses] = await Promise.all([
        pool.query('SELECT * FROM coffee_laboratories WHERE exporter_id = $1 AND status = $2', [exporterId, 'ACTIVE']),
        pool.query('SELECT * FROM coffee_tasters WHERE exporter_id = $1 AND status = $2', [exporterId, 'ACTIVE']),
        pool.query('SELECT * FROM competence_certificates WHERE exporter_id = $1 AND status = $2', [exporterId, 'ACTIVE']),
        pool.query('SELECT * FROM export_licenses WHERE exporter_id = $1 AND status = $2', [exporterId, 'ACTIVE']),
      ]);
      ...
      const prefillData = {
        ...
        licenseInfo: license ? {
          ...
          requestedCoffeeTypes: license.requested_coffee_types,
          requestedOrigins: license.requested_origins,
        } : null,
        competenceInfo: competenceCert ? {
          certificateNumber: competenceCert.certificate_number,
          issuedDate: competenceCert.issued_date,
          expiryDate: competenceCert.expiry_date,
          status: competenceCert.status,
        } : null,
        laboratoryInfo: laboratory ? {
          laboratoryName: laboratory.laboratory_name,
          certificationNumber: laboratory.certification_number,
          address: laboratory.address,
          certifiedDate: laboratory.certified_date,
          expiryDate: laboratory.expiry_date,
          hasRoastingFacility: laboratory.has_roasting_facility,
          hasCuppingRoom: laboratory.has_cupping_room,
          hasSampleStorage: laboratory.has_sample_storage,
        } : null,
        tasterInfo: taster ? {
          fullName: taster.full_name,
          proficiencyCertificateNumber: taster.proficiency_certificate_number,
          qualificationLevel: taster.qualification_level,
          certificateIssueDate: taster.certificate_issue_date,
          certificateExpiryDate: taster.certificate_expiry_date,
        } : null,
        qualificationStatus: {
          hasActiveProfile: profile.status === 'ACTIVE',
          hasActiveLicense: !!license,
          hasCompetenceCertificate: !!competenceCert,
          hasCertifiedLaboratory: !!laboratory,
          hasQualifiedTaster: !!taster,
          isFullyQualified: validation.isValid,
        },
        validation: {
          isValid: validation.isValid,
      ...
      logger.info('ESW pre-fill data retrieved', { userId, exporterId, isQualified: validation.isValid });

      res.json({
        success: true,
        data: prefillData,
        message: validation.isValid 
          ? 'You are qualified to submit ESW. Your information has been prepared.' 
          : 'Complete all pre-registration requirements to submit ESW.',
      });
    } catch (error: any) {
      logger.error('Failed to get ESW pre-fill data', { error: error.message, userId: req.user?.id });
      res.status(500).json({
        success: false,
        message: 'Failed to get ESW pre-fill data',
        error: error.message,
      });
    }
  };
