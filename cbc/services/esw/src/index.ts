...
// Log JWT_SECRET to verify it's loaded (first 20 chars only for security)
console.log('🔐 JWT_SECRET loaded:', process.env.JWT_SECRET ? `${process.env.JWT_SECRET.substring(0, 20)}... (${process.env.JWT_SECRET.length} chars)` : 'NOT SET');
...
app.use("/api/esw/auth", authRoutes); // Auth routes (no rate limiting for login)
