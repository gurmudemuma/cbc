...
const certificate_verification_service_1 = require("../src/services/certificate-verification.service");
const certificateVerificationService = new certificate_verification_service_1.CertificateVerificationService();
// Database connection
...
async function createTestCertificate(agencyCode, exporterData, exportDetails, approvalData, status = 'ACTIVE') {
...
async function cleanupTestData(submissionId) {
    ...
    try {
        await client.query('BEGIN');
        // Delete audit logs
        await client.query(`DELETE FROM esw_certificate_audit_log 
       WHERE certificate_id IN (SELECT certificate_id FROM esw_certificates WHERE submission_id = $1)`, [submissionId]);
        // Delete certificates
...
describe('ESW Certificate Verification - Property-Based Tests', () => {
    ...
    test('Property 12: Verification endpoint returns correct response for valid certificates', async () => {
        await fc.assert(fc.asyncProperty(agencyCodeArb, exporterDataArb, exportDetailsArb, approvalDataArb, async (agencyCode, exporterData, exportDetails, approvalData) => {
            ...
            try {
                ...
                const testData = await createTestCertificate(agencyCode, exporterData, exportDetails, approvalData, 'ACTIVE');
    ...
    test('Property 12: Verification endpoint returns NOT_FOUND for invalid certificates', async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 10, maxLength: 50 }).filter(s => !s.includes('CERT')), async (invalidCertNumber) => {
    ...
    test('Property 13: Verification endpoint returns REVOKED for revoked certificates', async () => {
        await fc.assert(fc.asyncProperty(agencyCodeArb, exporterDataArb, exportDetailsArb, approvalDataArb, async (agencyCode, exporterData, exportDetails, approvalData) => {
            ...
            try {
                ...
                const testData = await createTestCertificate(agencyCode, exporterData, exportDetails, approvalData, 'REVOKED');
                submissionId = testData.submissionId;
                const certificateNumber = testData.certificateNumber;
                // Verify certificate
                const verificationResult = await certificateVerificationService.verifyCertificate(certificateNumber);
    ...
    test('Property 14: Verification attempts are logged to audit log', async () => {
        await fc.assert(fc.asyncProperty(agencyCodeArb, exporterDataArb, exportDetailsArb, approvalDataArb, fc.ipV4(), fc.string({ minLength: 10, maxLength: 200 }), async (agencyCode, exporterData, exportDetails, approvalData, ipAddress, userAgent) => {
            let submissionId = null;
            try {
                // Create test certificate
                const testData = await createTestCertificate(agencyCode, exporterData, exportDetails, approvalData, 'ACTIVE');
                ...
                await certificateVerificationService.logVerificationAttempt(certificateNumber, ipAddress, userAgent, { testRun: true });
                // Verify audit log entry was created
                const auditResult = await pool.query(`SELECT * FROM esw_certificate_audit_log 
               WHERE certificate_id = $1 AND action = 'VERIFIED'
               ORDER BY timestamp DESC LIMIT 1`, [certificateId]);
    ...
    test('Verification works for certificates from all agencies', async () => {
        ...
        try {
            for (const agencyCode of VALID_AGENCY_CODES) {
                // Create test certificate
                const testData = await createTestCertificate(agencyCode, {
    ...
    test('Verification returns EXPIRED for expired certificates', async () => {
        ...
        try {
            // Create test certificate
            const testData = await createTestCertificate('ECTA', {
                exporterName: 'Test Exporter',
                exporterTin: '1234567890',
                businessName: 'Test Business'
            }, {
            ...
            await pool.query(`UPDATE esw_certificates 
...
//# sourceMappingURL=certificate-verification.property.test.js.map