...
export class LicenseController {
   ...
   */
  public reviewLicense = async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
      ...
      const { exporterLicenseNumber, licenseExpiryDate, exporterTIN, reviewedBy, notes } = req.body;
   ...
   */
  public issueLicense = async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
      const { exportId } = req.params;
      const user = req.user;
      ...
      const { licenseNumber, issuedBy, validUntil, notes } = req.body;
   ...
   */
  public getApprovedLicenses = async (_req: RequestWithUser, res: Response): Promise<void> => {
    try {
      const contract = this.fabricGateway.getExportContract();
      const exportService = createExportService(contract);
   ...
   */
  public getRejectedLicenses = async (_req: RequestWithUser, res: Response): Promise<void> => {
    try {
      const contract = this.fabricGateway.getExportContract();
      const exportService = createExportService(contract);
