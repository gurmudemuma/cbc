...
import { JWT_CONFIG } from '../../../shared/auth/jwt.config';
import { BlockchainUserService, createUserService } from '../../../shared/userService';
...
export class AuthController {
  ...
  constructor() {
    // Use shared JWT config for consistency across services
    this.JWT_SECRET = JWT_CONFIG.SECRET;
    this.JWT_EXPIRES_IN = JWT_CONFIG.ACCESS_TOKEN_EXPIRES_IN;
  ...
  public register = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const newUser = await userService.registerUser({
        ...
        password,
        email,
        organizationId: organizationId || 'ECTA-001',
        role: role || 'inspector',
      });

      // Generate token with shared format
      const token = jwt.sign(
        {
          id: newUser.id,
          username: newUser.username,
          organization: newUser.organizationId,
          organizationId: newUser.organizationId, // Keep for backward compatibility
          mspId: 'ECTAMSP',
          role: newUser.role,
      ...
      const token = jwt.sign(
        {
          ...
          organizationId: user.organizationId, // Keep for backward compatibility
          mspId: 'ECTAMSP',
          role: user.role,
          permissions: ['read', 'write'], // Default permissions
        },
