...
export class ContractController {
   ...
   */
  public reviewContract = async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
   ...
   */
  public verifyOrigin = async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
      const { exportId } = req.params;
      const user = req.user;
      ...
      const { originCertificateNumber, originRegion, verifiedBy, notes } = req.body;
  ...
  public approveContract = async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
      ...
      const { contractNumber, originCertificateNumber, approvedBy, notes } = req.body;
      ...
      res.json({
        success: true,
        message:
          'Contract approved. Export can proceed to Commercial Bank for document verification.',
        exportId,
        contractNumber,
   ...
   */
  public getApprovedContracts = async (_req: RequestWithUser, res: Response): Promise<void> => {
    try {
      const contract = this.fabricGateway.getExportContract();
      const exportService = createExportService(contract);
   ...
   */
  public getRejectedContracts = async (_req: RequestWithUser, res: Response): Promise<void> => {
    try {
      const contract = this.fabricGateway.getExportContract();
      const exportService = createExportService(contract);
