...
export class ExportController {
  ...
  public createExport = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!qualificationCheck.allowed) {
        logger.warn(`❌ Export creation blocked for ${exporterId}: ${qualificationCheck.reason}`);
      ...
      if (!exportData.exportId || !exportData.coffeeType || !exportData.quantity) {
        res.status(400).json({
          success: false,
      ...
      // The ECX service will handle the actual blockchain submission after verifying the lot
      const response = await axios.post(
        `${ECX_API_URL}/api/ecx/create-export`,
        {
          exportId: exportData.exportId,
          commercialbankId: 'commercial-bank', // Default for now
          exporterName: exportData.exporterName,
          exporterTIN: exportData.exporterTIN,
          exportLicenseNumber: exportData.exportLicenseNumber,
          coffeeType: exportData.coffeeType,
          quantity: Number(exportData.quantity),
          destinationCountry: exportData.destinationCountry,
          estimatedValue: Number(exportData.estimatedValue || 0),
        ...
        },
        {
          headers: {
            ...
            // Add auth token if needed for inter-service communication
          }
        }
      );

      logger.info(`✅ Export created via ECX: ${exportData.exportId} (Exporter: ${exporterId})`);

      res.status(201).json({
        success: true,
        message: 'Export request created successfully via ECX. Lot verified.',
        data: response.data
      });
    } catch (error: unknown) {
   ...
   */
  public getMyExports = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as any).user?.id;
      ...
      const result = await contract.evaluateTransaction('GetExportsByExporter', userId);
   ...
   */
  public getExportById = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
      ...
      const message = error instanceof Error ? error.message : 'Failed to fetch export';
   ...
   */
  public getExportHistory = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
      ...
      // Get history
      const historyResult = await contract.evaluateTransaction('GetExportHistory', id!);
   ...
   */
  public submitToECX = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
      ...
      const message = error instanceof Error ? error.message : 'Failed to submit to ECX';
   ...
   */
  public submitToECTA = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
      ...
      const message = error instanceof Error ? error.message : 'Failed to submit to ECTA';
   ...
   */
  public submitToBank = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
      ...
      res.status(200).json({
        success: true,
        message: 'Export submitted to Commercial Bank for document verification',
        data: { exportId: id, newStatus: 'BANK_DOCUMENT_PENDING' },
      });
    } catch (error: unknown) {
      logger.error('❌ Error submitting to Bank:', error);
      const message = error instanceof Error ? error.message : 'Failed to submit to Bank';
  ...
  public getDocumentStatus = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { getDocumentChecklist, getStageRequirements, getDocumentCompletionPercentage } =
        await import('../../../shared/documentTracking.service');

      const checklist = getDocumentChecklist(exportRecord);
      const stageRequirements = getStageRequirements(exportRecord, exportRecord.status);
      const completionPercentage = getDocumentCompletionPercentage(exportRecord);
   ...
   * Status: ECTA_QUALITY_REJECTED → ECTA_QUALITY_PENDING
   */
  public requestReinspection = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      ...
      if (exportRecord.createdBy !== userId) {
        res.status(403).json({
          success: false,
          message: 'Access denied. You can only request reinspection for your own exports.',
        });
        return;
      }

      // Verify status
      if (exportRecord.status !== 'ECTA_QUALITY_REJECTED') {
        res.status(400).json({
          success: false,
          message: `Cannot request reinspection. Current status: ${exportRecord.status}. Expected: ECTA_QUALITY_REJECTED`,
        });
        return;
      }

      // Submit to blockchain
      await contract.submitTransaction('RequestReinspection', id!, notes || '');

      logger.info(`✅ Reinspection requested for export ${id} by ${userId}`);

      res.status(200).json({
        success: true,
        message: 'Reinspection requested successfully',
        data: { exportId: id, newStatus: 'ECTA_QUALITY_PENDING' },
      });
    } catch (error: unknown) {
      logger.error('❌ Error requesting reinspection:', error);
      const message = error instanceof Error ? error.message : 'Failed to request reinspection';
   ...
   * Status: Any Rejected → DRAFT
   */
  public updateRejectedExport = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { coffeeType, quantity, destinationCountry, estimatedValue } = req.body;
      const userId = (req as any).user?.id;
      ...
      if (exportRecord.createdBy !== userId) {
        res.status(403).json({
          success: false,
          message: 'Access denied. You can only update your own exports.',
        });
        return;
      }
      ...
      if (!rejectedStatuses.includes(exportRecord.status)) {
        res.status(400).json({
          success: false,
          message: `Cannot update export. Current status: ${exportRecord.status}. Only rejected exports can be updated.`,
        });
        return;
      }
      ...
      await contract.submitTransaction(
        ...
        quantity?.toString() || '0',
        destinationCountry || '',
        estimatedValue?.toString() || '0'
      );

      logger.info(`✅ Rejected export ${id} updated by ${userId}`);

      res.status(200).json({
        success: true,
        message: 'Export updated successfully. Ready to resubmit.',
        data: { exportId: id, newStatus: 'DRAFT' },
      });
    } catch (error: unknown) {
      logger.error('❌ Error updating rejected export:', error);
   ...
   * Status: Any Rejected → DRAFT
   */
  public resubmitRejectedExport = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const userId = (req as any).user?.id;
      ...
      if (exportRecord.createdBy !== userId) {
        res.status(403).json({
          success: false,
          message: 'Access denied. You can only resubmit your own exports.',
        });
        return;
      }
      ...
      if (!rejectedStatuses.includes(exportRecord.status)) {
        res.status(400).json({
          success: false,
          message: `Cannot resubmit export. Current status: ${exportRecord.status}. Only rejected exports can be resubmitted.`,
        });
        return;
      }

      // Submit to blockchain
      await contract.submitTransaction('ResubmitRejectedExport', id!);

      logger.info(`✅ Rejected export ${id} resubmitted by ${userId}`);

      res.status(200).json({
        success: true,
        message: 'Export resubmitted successfully',
        data: { exportId: id, newStatus: 'DRAFT' },
      });
    } catch (error: unknown) {
      logger.error('❌ Error resubmitting export:', error);
      const message = error instanceof Error ? error.message : 'Failed to resubmit export';
