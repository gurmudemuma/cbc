...
export class AuthController {
  ...
  public login = async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
    try {
      ...
      // Local test credentials fallback (opt-in)
      if (config.ALLOW_LOCAL_TEST_CREDENTIALS) {
        const testUser = config.DEFAULT_TEST_EXPORTER_USERNAME;
        const testPass = config.DEFAULT_TEST_EXPORTER_PASSWORD;

        if (username === testUser && password === testPass) {
          const user = {
            id: config.DEFAULT_TEST_EXPORTER_ID,
            username: config.DEFAULT_TEST_EXPORTER_USERNAME,
            email: config.DEFAULT_TEST_EXPORTER_EMAIL,
            organizationId: config.DEFAULT_TEST_EXPORTER_ORG,
            role: 'exporter',
          } as any;
          ...
          res.status(200).json({
            success: true,
            message: 'Login successful (local test credentials)',
            data: {
              user: {
                id: user.id,
                username: user.username,
