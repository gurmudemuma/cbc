...
// Using the exporter-portal shared logger instance
...
export class ExporterController {
  ...
  public getQualificationStatus = async (
    ...
    try {
      await this.shared.getQualificationStatus(req as any, res, next);
    } catch (error: unknown) {
      logger.error('Failed to get qualification status (exporter controller):', error);
      const message = error instanceof Error ? error.message : 'Failed to get qualification status';
  ...
  public registerProfile = async (
    ...
    try {
      await this.shared.registerProfile(req as any, res, next);
    } catch (error: unknown) {
  ...
  public getProfile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      await this.shared.getProfile(req as any, res, next);
    } catch (error: unknown) {
   ...
   */
  public applyLicense = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      await this.shared.applyLicense(req as any, res, next);
    } catch (error: unknown) {
      logger.error('Failed to apply for export license (exporter controller):', error);
      const message = error instanceof Error ? error.message : 'Failed to apply for export license';
      res.status(500).json({ success: false, message, error: message });
    }
  };
  ...
  public registerLaboratory = async (
    ...
    try {
      await this.shared.registerLaboratory(req as any, res, next);
    } catch (error: unknown) {
      logger.error('Failed to register laboratory (exporter controller):', error);
      const message = error instanceof Error ? error.message : 'Failed to register laboratory';
      res.status(500).json({ success: false, message, error: message });
    }
  };
  ...
  public registerTaster = async (
    ...
    try {
      await this.shared.registerTaster(req as any, res, next);
    } catch (error: unknown) {
      logger.error('Failed to register taster (exporter controller):', error);
      const message = error instanceof Error ? error.message : 'Failed to register taster';
      res.status(500).json({ success: false, message, error: message });
    }
  };
  ...
  public applyCompetence = async (
    ...
    try {
      await this.shared.applyCompetence(req as any, res, next);
    } catch (error: unknown) {
      logger.error('Failed to apply for competence certificate (exporter controller):', error);
      const message =
        error instanceof Error ? error.message : 'Failed to apply for competence certificate';
      res.status(500).json({ success: false, message, error: message });
    }
  };
}
