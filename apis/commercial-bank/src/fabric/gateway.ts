...
export class FabricGateway {
  ...
  public async connect(): Promise<void> {
    try {
      ...
      if (!identity) {
        logger.info('Admin identity not found in wallet. Enrolling admin...', { service: 'CommercialBank' });
  ...
  private async enrollAdmin(wallet: Wallet): Promise<void> {
    try {
      ...
      logger.info(
        ...
        { service: 'FabricGateway' }
      );
    } catch (error) {
      logger.error(`Failed to enroll admin: ${error}`, { service: 'FabricGateway' });
      throw error;
    }
  }
