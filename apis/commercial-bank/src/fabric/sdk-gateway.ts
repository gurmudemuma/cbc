...
export class FabricSDKGateway {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      // Create wallet
      const walletPath = path.resolve(process.cwd(), process.env.WALLET_PATH || './wallet');
      ...
      if (!identity) {
        ...
        if (!identity) {
          throw new Error('No valid identity found. Please ensure admin identity exists.');
        }
      }

      // Use CommercialBankMSP as exporters operate under the bank's organization
      // The connection profile already specifies the correct MSP
      logger.info(`✅ Using identity: ${clientIdentityName} (External Client via Commercial Bank)`);
      logger.info(`🏛️  MSP: ${identity.mspId} (Commercial Bank)`);
      ...
      const exportChaincodeName = process.env.CHAINCODE_NAME_EXPORT || 'coffee-export';
      const userChaincodeName = process.env.CHAINCODE_NAME_USER || 'user-management';
      ...
      logger.info('🔐 External client access: Limited to exporter-specific functions only');

      this.isConnected = true;
      logger.info('🎉 Fabric SDK Gateway initialized successfully (External Client Mode)');
    } catch (error) {
  ...
  public getExportContract(): Contract {
    if (!this.exportContract) {
      throw new Error('Export contract not initialized. Call initialize() first.');
    }
  ...
  public getUserContract(): Contract {
    if (!this.userContract) {
      throw new Error('User contract not initialized. Call initialize() first.');
    }
   ...
   */
  public validateExternalAccess(functionName: string, contractType: 'export' | 'user'): boolean {
    ...
    const allowedUserFunctions = ['RegisterUser', 'GetUser', 'AuthenticateUser'];

    const allowedFunctions =
      contractType === 'export' ? allowedExportFunctions : allowedUserFunctions;

    if (!allowedFunctions.includes(functionName)) {
      logger.warn(`🚫 External client attempted to access unauthorized function: ${functionName}`);
      return false;
    }
  ...
  public async invokeContractSecurely(
    ...
    if (!this.validateExternalAccess(functionName, contractType)) {
      throw new Error(`Access denied: External client not authorized to call ${functionName}`);
    }

    // Get appropriate contract
    const contract = contractType === 'export' ? this.getExportContract() : this.getUserContract();
