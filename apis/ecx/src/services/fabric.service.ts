...
export class FabricService {
  ...
  async connect(): Promise<void> {
    try {
      ...
      if (!identity) {
        throw new Error('ECX admin identity not found in wallet. Please enroll first.');
      }
      ...
      this.contract = this.network.getContract(process.env.CHAINCODE_NAME || 'coffee-export');
  ...
  async getExportsByStatus(status: string): Promise<any[]> {
    try {
      ...
      const result = await contract.evaluateTransaction('GetExportsByStatus', status);
  ...
  async rejectECXVerification(exportId: string, reason: string): Promise<string> {
    try {
      ...
      const result = await contract.submitTransaction('RejectECXVerification', exportId, reason);
