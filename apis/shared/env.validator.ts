...
class EnvValidator {
  ...
  public printSummary(): void {
    ...
    logger.info('Configuration Summary', {
      ...
      port: config.PORT,
      organization: `${config.ORGANIZATION_NAME} (${config.ORGANIZATION_ID})`,
      mspId: config.MSP_ID,
      channel: config.CHANNEL_NAME,
      chaincodeExport: config.CHAINCODE_NAME_EXPORT,
      chaincodeUser: config.CHAINCODE_NAME_USER,
      ipfs: `${config.IPFS_PROTOCOL}://${config.IPFS_HOST}:${config.IPFS_PORT}`,
