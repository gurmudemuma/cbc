...
export class EctaPreRegistrationService {
  ...
  public async validateExporter(exporterId: string): Promise<ExporterValidation> {
    ...
    if (profile.businessType !== 'FARMER') {
      ...
      if (!laboratory) {
        ...
        const labValid =
          laboratory.status === 'ACTIVE' && new Date(laboratory.expiryDate) > new Date();
    ...
    if (!competenceCert) {
      ...
      const certValid =
        competenceCert.status === 'ACTIVE' && new Date(competenceCert.expiryDate) > new Date();
    ...
    if (!exportLicense) {
      ...
      const licenseValid =
        exportLicense.status === 'ACTIVE' && new Date(exportLicense.expiryDate) > new Date();
  ...
  private async getCompetenceCertificate(
    exporterId: string
