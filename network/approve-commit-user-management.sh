...
setGlobals 1
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.coffee-export.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --peerAddresses localhost:7051 --tlsRootCertFiles "${PEER0_EXPORTERBANK_CA}" --peerAddresses localhost:9051 --tlsRootCertFiles "${PEER0_NATIONALBANK_CA}" --peerAddresses localhost:11051 --tlsRootCertFiles "${PEER0_NCAT_CA}" --peerAddresses localhost:13051 --tlsRootCertFiles "${PEER0_SHIPPINGLINE_CA}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE}
