...
echo -e "${GREEN}Installing chaincode v1.9 on all peers${NC}"

export PACKAGE_FILE="/opt/gopath/src/github.com/hyperledger/fabric/chaincode/ecta-v1.9.tar.gz"
...
install_on_peer() {
    ...
    echo -e "${YELLOW}Installing on ${PEER}...${NC}"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "${YELLOW}Note: May already be installed on ${PEER}${NC}"
    fi
}
