...
# Approve for ECTA with ECTA-only policy
...
# Approve for Bank with ECTA-only policy
echo -e "${YELLOW}Approving for BankMSP...${NC}"
...
# Approve for NBE with ECTA-only policy
echo -e "${YELLOW}Approving for NBEMSP...${NC}"
...
# Approve for Customs with ECTA-only policy
echo -e "${YELLOW}Approving for CustomsMSP...${NC}"
...
# Approve for Shipping with ECTA-only policy
echo -e "${YELLOW}Approving for ShippingMSP...${NC}"
...
# Commit with ECTA-only policy (but include all peers for commit)
...
peer lifecycle chaincode commit \
    ...
    --tlsRootCertFiles ${CRYPTO_PATH}/peerOrganizations/shipping.example.com/peers/peer0.shipping.example.com/tls/ca.crt 2>&1 | tail -5
