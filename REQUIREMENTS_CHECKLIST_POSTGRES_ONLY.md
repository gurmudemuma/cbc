# Coffee Blockchain Consortium - PostgreSQL-Only Requirements Checklist

## Project Overview
The Coffee Blockchain Consortium (CBC) is a comprehensive export management system for Ethiopian coffee exports. This version focuses on a **PostgreSQL-only end-to-end system** without blockchain integration. Blockchain features will be added later.

---

## 1. SYSTEM ARCHITECTURE REQUIREMENTS

### 1.1 Backend Services (PostgreSQL-Only)
...
- [x] **Shared Library** (`/api/shared`)
  ...
  - Components:
    ...
    - Services (export, audit, email, WebSocket)
    - Validation schemas
...
- [x] **Migration 002: Documents Table**
  ...
  - Purpose: Document metadata storage (local file system or S3)
  - Indexes: entity_id, entity_type, document_type, is_active, uploaded_by
  - Status: Implemented
  - **Note: IPFS removed, use local storage or S3**
...
- [x] FILE_STORAGE_PATH (local directory or S3 bucket)
...
- [x] redis (4.7.1) - Redis client (optional for caching)
...
- ❌ fabric-network (2.2.20) - **REMOVED**
- ❌ fabric-ca-client (2.2.20) - **REMOVED**
- ❌ ipfs-http-client (60.0.1) - **REMOVED**
...
- [x] **API Services Dockerfile**
  ...
  - **Blockchain: REMOVED from Dockerfile**
...
- [x] **Docker Compose Configuration**
  ...
  - Status: Fully configured
  - **Blockchain: REMOVED (no Fabric services)**
  - **IPFS: REMOVED**
...
## 12. DOCUMENTATION & CONFIGURATION FILES
...
### 12.2 Documentation
- [x] **REQUIREMENTS_CHECKLIST_POSTGRES_ONLY.md** - This file
- [x] **README.md** - Project documentation (frontend)

---

## 13. WORKFLOW & PROCESS REQUIREMENTS

### 13.1 Export Workflow Stages
...
### 13.2 Pre-Registration Requirements
...
## 16. FILE STORAGE CONFIGURATION

### 16.1 Document Storage (PostgreSQL-Only)
- [x] **Local File System Storage**
  - Documents stored in local directory
  - Path configured via FILE_STORAGE_PATH
  - Metadata stored in preregistration_documents table
  - File references in database

- [x] **Alternative: S3 Storage**
...
- [ ] File storage directory needs to be created and writable
...
- [ ] Document storage path must be accessible to all API services
...
# Check audit logs
SELECT * FROM preregistration_audit_log ORDER BY timestamp DESC LIMIT 10;
...
### ✅ Completed Requirements (PostgreSQL-Only)
...
- All required dependencies listed (Blockchain removed)
...
### 🚀 Ready for PostgreSQL-Only Deployment
...
### 📋 Total Requirements Tracked (PostgreSQL-Only)
- **Architecture Components**: 7 API services + 1 frontend
- **Database Tables**: 20+ tables across 4 migrations
- **Environment Variables**: 50+ configuration options (Blockchain removed)
- **Dependencies**: 45+ backend + 20+ frontend packages (Blockchain removed)
- **Services**: 11 core services (IPFS removed)
...
When ready to add blockchain features:
1. Add Hyperledger Fabric dependencies
2. Configure Fabric network
3. Implement Fabric Gateway
4. Add chaincode integration
5. Update API services to use blockchain
6. Migrate data to blockchain as needed

---

**Document Version**: 1.0 (PostgreSQL-Only)  
**Last Updated**: 2024  
**Status**: ✅ Complete - PostgreSQL-only system ready for development
