...
class VerificationService {
    ...
    async performCreditCheck(buyer) {
        try {
            logger_1.logger.info(`Performing credit check for ${buyer.company_name}`);
            if (process.env.CREDIT_CHECK_API_KEY) {
                const response = await axios_1.default.post(process.env.CREDIT_CHECK_API_URL, {
            ...
            logger_1.logger.error('Credit check error:', error);
            return {
                status: 'ERROR',
                error: error.message,
                provider: 'CreditCheckAPI',
    ...
    async performSanctionsScreening(buyer) {
        try {
            logger_1.logger.info(`Performing sanctions screening for ${buyer.company_name}`);
            if (process.env.SANCTIONS_API_KEY) {
                const response = await axios_1.default.post(process.env.SANCTIONS_API_URL, { name: buyer.company_name, country: buyer.country, type: 'COMPANY' }, {
                    headers: {
            ...
            logger_1.logger.error('Sanctions screening error:', error);
            return {
                status: 'ERROR',
                error: error.message,
                provider: 'SanctionsAPI',
    ...
    async verifyCompanyRegistry(buyer) {
        try {
            logger_1.logger.info(`Verifying company registry for ${buyer.company_name}`);
            if (process.env.COMPANY_REGISTRY_API_KEY) {
                const response = await axios_1.default.get(`${process.env.COMPANY_REGISTRY_API_URL}/search`, {
                    params: {
            ...
            logger_1.logger.error('Company registry verification error:', error);
            return {
                status: 'ERROR',
                error: error.message,
                provider: 'CompanyRegistryAPI',
    ...
    async saveVerificationRecords(pool, buyerId, verificationResults) {
        ...
        try {
            ...
            for (const [type, result] of Object.entries(verificationResults)) {
                ...
                await client.query(`INSERT INTO buyer_verification_records (
    ...
    async updateBuyerVerificationStatus(pool, buyerId, verificationResults) {
        ...
        await pool.query(`UPDATE buyer_registry
       SET verification_status = $1,
           verification_date = CURRENT_TIMESTAMP,
           sanctions_check_status = $2,
           sanctions_check_date = CURRENT_TIMESTAMP,
           updated_at = CURRENT_TIMESTAMP
       WHERE buyer_id = $3`, [verificationStatus, verificationResults.sanctionsCheck?.status ?? 'PENDING', buyerId]);
    }
