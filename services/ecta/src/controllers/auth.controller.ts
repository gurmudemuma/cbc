...
console.log('🔄 AuthController module loaded - bcrypt type:', typeof bcrypt);
const logger = createLogger('ECTAAuthController');
...
export class AuthController {
  ...
  public login = async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
    ...
    logger.info('bcrypt object', { bcrypt: typeof bcrypt, hasCompare: typeof bcrypt?.compare });
    ...
    try {
      ...
      if (!user.password_hash) {
        logger.error('User has no password_hash', { userId: user.id, username: user.username });
        throw new AppError(ErrorCode.UNAUTHORIZED, 'Invalid credentials', 401);
      }

      if (!bcrypt || typeof bcrypt.compare !== 'function') {
        logger.error('bcrypt.compare is not available', { bcryptType: typeof bcrypt });
        throw new AppError(ErrorCode.INTERNAL_SERVER_ERROR, 'Authentication system error', 500);
      }
