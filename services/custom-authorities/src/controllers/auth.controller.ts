...
const logger = createLogger('CustomAuthoritiesAuthController');
...
export class AuthController {
  ...
  public register = async (_req: Request, res: Response, _next: NextFunction): Promise<void> => {
    const pool = getPool();
    if (!pool) {
    ...
    try {
      const { username, password, email, organizationId, role } = _req.body;
      ...
      const result = await client.query(
        ...
        [username, email, hashedPassword, organizationId || 'CUSTOM_AUTHORITIES', role || 'customs_officer']
      );
