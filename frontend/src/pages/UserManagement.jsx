...
import { Box, Button, Card, CardContent, Dialog, DialogTitle, DialogContent, DialogActions, Grid, Stack, TextField, Typography, Chip, Table, TableHead, TableRow, TableCell, TableBody, Paper } from '@mui/material';

const DEFAULT_EXPORTER_ORG = 'EXPORTER-BANK-001';
...
const UserManagement = ({ user }) => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await apiClient.get(`/portal/users`, { params: { organizationId: DEFAULT_EXPORTER_ORG } });
  ...
  const createUser = async () => {
    try {
      ...
      await apiClient.post('/portal/users', {
        ...
        organizationId: DEFAULT_EXPORTER_ORG,
  ...
  const toggleActive = async (id, isActive) => {
    try {
      ...
      await apiClient.patch(`/portal/users/${id}/${path}`);
      fetchUsers();
    } catch (e) {
    ...
    <Box sx={{ p: 3 }}>
      <Grid container spacing={3}>
        <Grid item xs={12} md={9}>
          ...
          <Typography variant="subtitle1" sx={{ mb: 3 }}>Managed by National Bank</Typography>
          ...
          <Card sx={{ mb: 3 }}>
            <CardContent>
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Button variant="contained" startIcon={<Plus />} onClick={() => setOpen(true)}>New User</Button>
              </Stack>
            </CardContent>
          </Card>
                ...
                <TableBody>
                  {users.map(u => (
                    <TableRow key={u.id} hover>
                      ...
                      <TableCell>
                        <Chip size="small" color={u.isActive ? 'success' : 'default'} icon={u.isActive ? <CheckCircle size={14} /> : undefined} label={u.isActive ? 'Active' : 'Inactive'} />
                      </TableCell>
                      <TableCell align="right">
                        <Stack direction="row" spacing={1} justifyContent="flex-end">
                          <Button size="small" variant="outlined" color={u.isActive ? 'error' : 'success'} startIcon={u.isActive ? <Ban size={14} /> : <Shield size={14} />} onClick={() => toggleActive(u.id, u.isActive)}>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={createUser} variant="contained" disabled={loading || !form.username || !form.email || !form.password}>Create</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
