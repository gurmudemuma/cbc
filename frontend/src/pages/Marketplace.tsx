  ...
  const handleExpressInterest = async (opportunity: BuyerOpportunity) => {
    try {
      ...
      const contractData = {
        ...
        coffeeType: opportunity.coffee_type,
        originRegion: opportunity.origin_preferences?.[0] || 'Ethiopia', // Use first preference or default
        quantity: parseFloat(opportunity.quantity_min) || 500, // Start with minimum quantity
        unitPrice: parseFloat(opportunity.target_price_min) || 0, // Start with minimum price
        currency: opportunity.currency || 'USD',
        paymentTerms: opportunity.preferred_payment_terms?.[0] || 'LC_AT_SIGHT',
        paymentMethod: 'LC',
        incoterms: opportunity.preferred_incoterms?.[0] || 'FOB',
        deliveryDate: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 90 days from now
        portOfLoading: 'Djibouti Port', // Default Ethiopian export port
        portOfDischarge: opportunity.destination_port || 'TBD',
        governingLaw: 'Ethiopian Law',
        arbitrationLocation: 'Addis Ababa, Ethiopia',
        arbitrationRules: 'ICC Rules',
        contractLanguage: 'English',
        forceMajeureClause: 'Standard Force Majeure Clause as per ICC terms',
        qualityGrade: opportunity.quality_grade_min || 'Grade 1',
        specialConditions: `Based on marketplace opportunity: ${opportunity.title}. Buyer: ${opportunity.buyer_company_name || 'International Buyer'}. Requirements: ${opportunity.certifications_required?.join(', ') || 'None specified'}.`,
        certificationsRequired: opportunity.certifications_required || []
      };
      ...
      if (draftResponse.data.success) {
        ...
        throw new Error(draftResponse.data.error || 'Failed to create contract draft');
      }
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Failed to create contract draft. Please try again.';
