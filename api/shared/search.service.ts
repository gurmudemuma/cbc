...
export class SearchService {
  ...
  private applyFilters(exports: any[], criteria: SearchCriteria): any[] {
    ...
    if (criteria.destinationCountry) {
      filtered = filtered.filter((exp) =>
        exp.destinationCountry.toLowerCase().includes(criteria.destinationCountry!.toLowerCase())
      );
    }
  ...
  public getFacets(exports: ExportRequest[]): {
    ...
    exports.forEach((exp) => {
      ...
      if (exp.qualityGrade) {
        qualityGrades[exp.qualityGrade] =
          (qualityGrades[exp.qualityGrade] || 0) + + 1;
      }
  ...
  public buildCriteriaFromParams(params: Record<string, any>): SearchCriteria {
    ...
    if (params.status) {
      criteria.status = params.status.includes(',')
        ? params.status.split(',')
        : params.status;
    }
