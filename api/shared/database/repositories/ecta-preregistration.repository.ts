...
export class EctaPreRegistrationRepository {
  ...
  async createExporterProfile(profile: Omit<ExporterProfile, 'exporterId' | 'createdAt' | 'updatedAt'>): Promise<ExporterProfile> {
    const query = `
  ...
  async getExporterProfileByTin(tin: string): Promise<ExporterProfile | null> {
    const query = 'SELECT * FROM exporter_profiles WHERE tin = $1';
    const result = await this.pool.query(query, [tin]);
    return result.rows[0] ? this.mapExporterProfile(result.rows[0]) : null;
  }
  ...
  async updateExporterProfile(
    exporterId: string,
    updates: Partial<Omit<ExporterProfile, 'exporterId' | 'userId' | 'createdAt' | 'updatedAt' | 'status'>>
  ): Promise<ExporterProfile> {
    const allowedFields = [
      'businessName', 'tin', 'registrationNumber', 'businessType',
      'minimumCapital', 'officeAddress', 'city', 'region',
    ...
    if (updateFields.length === 0) {
      // No valid fields to update, return current profile
      const current = await this.getExporterProfileById(exporterId);
      if (!current) throw new Error('Profile not found');
      return current;
    }

    values.push(exporterId);
    const query = `
      UPDATE exporter_profiles 
      SET ${updateFields.join(', ')}, updated_at = CURRENT_TIMESTAMP
      WHERE exporter_id = $${paramIndex}
      RETURNING *
    `;
  ...
  async createLaboratory(laboratory: Omit<CoffeeLaboratory, 'laboratoryId' | 'createdAt' | 'updatedAt'>): Promise<CoffeeLaboratory> {
    const query = `
      ...
      DO UPDATE SET
        ...
        has_roasting_facility = EXCLUDED.has_roasting_facility,
  ...
  async createTaster(taster: Omit<CoffeeTaster, 'tasterId' | 'createdAt' | 'updatedAt'>): Promise<CoffeeTaster> {
    const query = `
      ...
      DO UPDATE SET
        ...
        qualification_document = EXCLUDED.qualification_document,
  ...
  // Lightweight competence application insertion (used by controller)
  async createCompetenceApplication(application: {
    exporterId: string;
    ...
    const values = [
      application.exporterId,
      application.applicationReason || null,
      JSON.stringify(application.additionalDocuments || []),
      application.facilityDescription || null,
      'PENDING', // Force PENDING status to match constraints and ECTA query
      application.applicationDate,
      application.applicantUserId,
    ];
  ...
  async createExportLicense(license: Omit<ExportLicense, 'licenseId' | 'createdAt' | 'updatedAt'>): Promise<ExportLicense> {
    const query = `
      INSERT INTO export_licenses (
  ...
  async createCoffeeLot(lot: Omit<CoffeeLot, 'lotId' | 'createdAt' | 'updatedAt'>): Promise<CoffeeLot> {
    const query = `
  ...
  async createSalesContract(contract: Omit<SalesContract, 'contractId' | 'createdAt' | 'updatedAt'>): Promise<SalesContract> {
    const query = `
  ...
  async storeFailedForwarding(payload: {
    ...
    const query = `
      INSERT INTO failed_forwardings (
        exporter_id, eic_registration_number, payload, error, timestamp
      ) VALUES ($1, $2, $3, $4, $5)
    `;

    const values = [
      payload.exporterId,
      payload.eicRegistrationNumber,
