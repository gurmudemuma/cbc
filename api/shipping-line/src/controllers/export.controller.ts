...
export class ExportController {
  ...
  public getReadyForShipment = async (_req: RequestWithUser, res: Response, _next: NextFunction): Promise<void> => {
    try {
      const contract = this.fabricGateway.getExportContract();
      const exportService = createExportService(contract);
      ...
      res.status(500).json({ success: false, message: 'Failed to fetch ready shipments', error: error.message });
    }
  };

  public scheduleShipment = async (req: RequestWithUser, res: Response, _next: NextFunction): Promise<void> => {
    try {
      const { exportId } = req.params;
      if (!exportId) {
        res.status(400).json({ success: false, message: 'Export ID is required' });
      ...
      const { transportIdentifier, departureDate, arrivalDate, transportMode, documentCIDs } = req.body;
      const contract = this.fabricGateway.getExportContract();
      const exportService = createExportService(contract);
      await exportService.scheduleShipment(exportId, { transportIdentifier, departureDate, arrivalDate, transportMode, documentCIDs });
      res.json({ success: true, message: 'Shipment scheduled' });
    } catch (error: any) {
      res.status(500).json({ success: false, message: 'Failed to schedule shipment', error: error.message });
    }
  };

  public markAsShipped = async (req: RequestWithUser, res: Response, _next: NextFunction): Promise<void> => {
    try {
      const { exportId } = req.params;
      if (!exportId) {
        res.status(400).json({ success: false, message: 'Export ID is required' });
      ...
      res.status(500).json({ success: false, message: 'Failed to mark as shipped', error: error.message });
    }
  };

  public markAsArrived = async (req: RequestWithUser, res: Response, _next: NextFunction): Promise<void> => {
    try {
      const { exportId } = req.params;
      if (!exportId) {
        res.status(400).json({ success: false, message: 'Export ID is required' });
  ...
  public confirmDelivery = async (req: RequestWithUser, res: Response, _next: NextFunction): Promise<void> => {
    try {
      const { exportId } = req.params;
      if (!exportId) {
        res.status(400).json({ success: false, message: 'Export ID is required' });
